# -*- coding: utf-8 -*-
"""
    Simulated Nanospring oscillations Brownian Diffusion
    ============================
    The SimBrownian class generates synthetic images corresponding to particles performing a thermal Brownian
    motion on top of an actuated oscillatory motion.
    This class was designed in order to make explicit the real parameters of the particles (i.e. the diffusion
...
class SimBrownian:
    ...
    omega_spring = 1.0  # oscillation frequency in 1/s
    ...
    time_step = 0.03
    #: absolute time from the beginning of the simulation
    t_axis = 0.0
    ...
    def __init__(self, camera_size: tuple = (200, 250)):
    ...
    def gen_image(self):
        ...
        if self.current_frame < self.frames_to_accumulate or not self.frames_to_accumulate:
            ...
            for p in positions:
                ...
                pix_x = int(p[0])
                pix_y = int(p[1])
                s_img = (np.sin(self.omega_spring*self.t_axis)**2+1) / (np.sqrt(2 * np.pi)*self.psf_width) * np.exp(-((xx - p_x) ** 2 + (yy - p_y) ** 2)/(2*self.psf_width**2))
